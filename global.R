...
# setwd("C:/Users/skick/Desktop/CKM_Zipcode/")

zip.data = fread("finaldata2.csv")
...
# 
# zip.data = zip.data[Zip %in% nyczips]

zip.data$Avg.NSales = NULL

#Make range (0,100) for each numeric column
zip.data[,c("Population")] =            round(zip.data[,c("Population")], 2)*100
zip.data[,c("Avg.Crimes.by.Pop")] =     round(zip.data[,c("Avg.Crimes.by.Pop")], 2)*100
zip.data[,c("Avg.Fires.by.Pop")] =      round(zip.data[,c("Avg.Fires.by.Pop")], 2)*100
zip.data[,c("Avg.Complaints.by.Pop")] = round(zip.data[,c("Avg.Complaints.by.Pop")], 2)*100
zip.data[,c("Avg.Price")] =             round(zip.data[,c("Avg.Price")], 2)*100
zip.data[,c("Avg.PricePerSqFt")] =      round(zip.data[,c("Avg.PricePerSqFt")], 2)*100


# ------------------------------ shape file ------------------------------

#set temporary directory to unzip
tmp <- tempdir()
unzip("ZIP_CODE_040114.zip", exdir = tmp)

#read in the shapefile
nyc <- readOGR(dsn = tmp, layer = "ZIP_CODE_040114", encoding = "UTF-8")

#convert the data so it projects on map correctly 
PRO <- sp::CRS('+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0')
DAT <- sp::spTransform(nyc,PRO)

#merge data with shapefile
DAT = merge(DAT, zip.data, by.x= 'ZIPCODE', by.y = 'Zip')
